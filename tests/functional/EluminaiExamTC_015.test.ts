...
/*est(`@Smoke Verify Elumina Login`, async ({ eluminaLoginPage, eluminaHomePage, eluminaExamPage, webActions }) => { 
...
test(`@Smoke Verify Elumina RegistrationInv`, async ({ eluminaLoginPage,eluminaRegInvPage,webActions }) => {
    await test.step(`Navigate to Application`, async () => {
        await eluminaLoginPage.navigateToURL();
    });
    await test.step(`Login to Elumina application`, async () => {
