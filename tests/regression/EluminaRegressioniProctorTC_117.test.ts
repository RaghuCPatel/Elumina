...
test(`@Regression1 Validation of Proctoring Exam Event > Screen Share Enabled`, async ({ eluminaProctorCand, eluminaLoginPage, eluminaProctorReg, webActions }) => {
    ...
    await test.step('Admin logging into Application', async () => {
        ...
        await newPage.locator('(//div[contains(text(),"Screen Share Enabled")])[1]').isVisible();
        let screenshareVerified = await newPage.locator('(//div[contains(text(),"Screen Share Enabled")])[1]').textContent();
        console.log(screenshareVerified);
